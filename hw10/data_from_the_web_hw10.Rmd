...
Finally, it would be also useful to create an object of the type `list` only with the species names, to retrieve information in the next part...

```{r}
ubcbirdlist <- ubcbirds2$sciName
```
...
This object is basically a list with two elements `$meta` which stands for the number of records found and `$data` which contains the information we asked PLOS for... So, this is useful... Let's get the information for the 17 species in our list...

Now, lets get the data for the 17 species in our UBC-bird-list, I will define a function that run the query and retrieve the information for each species name, and that returns a dataframe with the name of the species, the number of papers published for each one, and all the rest of the information we want in a list... I have to include a `sleep time` parameter, because there is maximum of 10 request per minute on the website.
